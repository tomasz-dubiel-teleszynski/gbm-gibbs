...
for iBank = 1:nBanks
    ...
    for j = 1:J
        [bc,xas] = bankcost(cMUnii(idx,j),cSIGnii(idx,j),bCvg(iBank,1),bCvg(iBank,2),r);
        XASbnii(iBank,j) = xas;
         BCbnii(iBank,j) = bc;
     lastTIbnii(iBank,j) = TIb(end,iBank)/1e6;
        ATTAnii(iBank,j) = log(XASbnii(iBank,j)/lastTIbnii(iBank,j))/(cMUnii(idx,j)-0.5*cSIGnii(idx,j)^2);
    end
    
end

cMUnii = flipud(cMUnii(:,2:end)');
cSIGnii = flipud(cSIGnii(:,2:end)');
